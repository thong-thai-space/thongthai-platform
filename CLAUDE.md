...
**Provider neutrality (plan §6.4).** The product's core promise is vendor-neutral advice. The `ai` module is a **Provider Router**: one port (`AiProviderPort`) with swappable adapters — Claude (`AiProviderAdapter`), OpenAI (`OpenAiProviderAdapter`), Gemini (`GeminiProviderAdapter`). The active provider is chosen by `AI_PROVIDER` env (`claude` default | `openai` | `gemini`) via `createAiProvider` — switching is config-only, no code change, and the RAG module reuses the same port. Note: callers pass a Claude-centric `AI_MODEL`; non-Claude adapters substitute their own model (`OPENAI_MODEL` / `GEMINI_MODEL`, with defaults). RAG (GĐ2) uses **pgvector** (Postgres extension), not a separate vector DB.
