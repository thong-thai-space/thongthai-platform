...
import type { Message, TaskStatus, TaskPriority } from '@/types';
...
const taskStatusIconColors: Record<string, string> = {
  TODO: 'text-gray-400',
...
export default function MemberProjectDetailPage() {
  ...
  if (isLoading) {
    return (
      <>
        <MemberHeader title="Project Details" />
    ...
    return (
      <>
        ...
        <div className="flex flex-1 flex-col items-center justify-center text-muted-foreground">
          ...
          <Link href="/member/projects" className="mt-2 text-primary hover:underline">
            Back to list
          </Link>
        </div>
      </>
  ...
  return (
    <>
      <MemberHeader title={project.name} />
      <main className="flex-1 overflow-y-auto p-6">
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <InfoCard
            ...
            label="Client"
            value={project.client?.name || 'Not assigned'}
          />
        </div>
        ...
        <section className="tts-workspace-surface mb-6 p-5">
          ...
          {myTasks.length === 0 ? (
            <p className="text-sm text-muted-foreground">No tasks assigned to you in this project.</p>
          ) : (
              ...
              {myTasks.map((task) => (
                ...
                >
                  ...
                  <select
                    ...
                    className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium ${taskStatusColors[task.status]} border-0`}
                  >
...
function TaskStatusIcon({ status }: { status: string }) {
  const colorClass = taskStatusIconColors[status] || 'text-gray-400';
  if (status === 'DONE') {
