...
async function fetchBlogSlugs(): Promise<BlogSitemapEntry[]> {
  const slugs = await serverApiGet<BlogSitemapEntry[]>("/blog/sitemap", {
    next: { revalidate: 3600 },
  });
  // Guard the shape: never let a bad payload break the sitemap prerender.
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    PUBLIC_ROUTES.map((route) => {
      ...
      const languages = Object.fromEntries(
        routing.locales.map((alt) => [
          alt,
          `${SITE_URL}${localizedPath(alt, route)}`,
        ]),
      );
  ...
  const blogRoutes: MetadataRoute.Sitemap = blogSlugs.map(
